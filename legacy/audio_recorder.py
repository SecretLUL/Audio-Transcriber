...
class MegaAudioKiRecorder:
    def __init__(self, root):
        self.root = root
        self.root.title("Lokal Audio KI-Recorder (Mic + System + Whisper)")
    ...
    def init_ki_hardware(self):
        ...
        self.hardware_string = "CPU / Vulkan via whisper.cpp"
    ...
    def stop_recording(self):
        ...
        if mic_mono_16k is not None or loop_mono_16k is not None:
            ...
            try:
                ...
                # Falls Live-Transkription an war: Transkript ist sofort ohne Wartezeit fertig!
                if self.live_transcribe_var.get() and self.last_live_text.strip():
    ...
    def run_whisper_transcription(self, audio_path):
        try:
            ...
            # 3. Transkription starten via whisper-cli.exe (-di für Stereo-Sprechererkennung: [Du] vs [Desktop])
            ...
            if not formatted_text.strip():
                formatted_text = "[Audio verarbeitet, aber es wurde kein gesprochener Text erkannt.]"
